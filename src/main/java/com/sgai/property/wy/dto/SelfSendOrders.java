...
public class SelfSendOrders extends Thread{
    ...
    public void run() {
        while(true)
        {
            try {
                ...
                {
                    //业务逻辑处理块                  
                	repairInformationService.selfSendOrder(loginUser);
                	System.out.println("自动派单执行中");
                    //循环时间
                    Thread.sleep(period);
                }
                //当flag为false时，线程休息中
