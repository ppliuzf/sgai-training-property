...
$(function () {
	...
	if (-1 == WebVideoCtrl.I_CheckPluginInstall()) {
	 ...
	 //初始化视频
    if(getParams("ip")!=null){
        iWndowType = 1;
    }else{
    	iWndowType = 2;
    }
	// 初始化插件参数及插入插件
	WebVideoCtrl.I_InitPlugin(950, 570, {
    ...
    if(getParams("ip")!=null){
    	$("#loginip").val(getParams("ip"));
        clickLogin();
    }else{
    	initTree(getParams("token"));
    }
});
//获取摄像头ip值
...
function initTree(token){
	...
	$.ajax({
		headers : {"token" : token},
		url : 'http://10.3.153.51:9090/admin/mdm/devicesuseinfo/mdmDevicesUseInfo/getVideoTree',
		type : 'POST',
		...
		success : function(data) {
	        var tree = $.fn.zTree.init($("#videoTree"), setting, data.data);
	        tree.expandAll(true);
		},
		error : function() {
...
// 登录
function clickLogin() {
	var videoId = $('#videoId', parent.document).val();
	$("#loginip").val(videoId);
