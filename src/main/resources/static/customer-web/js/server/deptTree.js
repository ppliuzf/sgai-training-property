...
var deptTree = {
    init: function (config) {
        ...
        $.pop({
            ...
            content: template('common/dept-tree-view', {
                typeName: typeName
            }),
            noIcon: true,
            success: function () {
    ...
    act: function () {
        ...
        function getDeptData(config) {
            ...
            $.getData({
                url: self.opt.deptUrl || '/orgDept/getOrgTree',
                query: query
            }, function (data) {
                renderDeptTree(data);
            });
        ...
        function isSelectAll() {
            if (self.opt.onlyDept) {
                $('.js-dept-select-all input').prop('disabled', true);
                return;
            }
        ...
        function renderDeptTree(data, isSearch) {
            var html = template('common/dept-tree-list', {
                items: data,
                isSearch: isSearch,
                type: self.opt.type,
            ...
            setTimeout(function () {
                $('.dept-selector .content').css({
                    top: $('.js-dept-path').outerHeight() + 74 + 30
                });
            }, 200);
            getSelectedItems();
        }
        ...
        $('.dept-selector').on('click', '.clicker', function () {
            ...
            if ($inp.is(':checked')) {
                ...
                if (self.opt.onlyDept) {
                    $par.siblings().find('input').prop('disabled', false);
                    $par.siblings().find('.js-dept-inside').removeClass('disabled');
                }
            } else {
                $inp.prop('checked', true);
                $par.find('.js-dept-inside').addClass('disabled');
                if (self.opt.onlyDept) {
                    $par.siblings().find('input').prop('disabled', true);
                    $par.siblings().find('.js-dept-inside').addClass('disabled');
                }
            }
            getSelectValue($par);
            isSelectAll();
        ...
        $('.dept-selector').on('click', '.js-dept-path a', function () {
            ...
            if (!$this.hasClass('disabled')) {
                ...
                getDeptData({
                    deptId: $this.data('id') || self.opt.deptId
                });
                return false;
            }
        });
