$(function () {
    ...
    function getList() {
        $.getData({
            url: '/task/taskStatMonthByExecutor',
            query: {
                startTime: startDate,
                endTime: endDate,
                type: type ? type: 0
    ...
    $(document).on('click', '.taskBtn', function () {
        ...
        if($(this).text() == '任务详情'){
            ...
            window.location.href = 'task-detail.html?type=1&tpId='+id+'&record_id='+record+'&state='+stateId+'&dt=' + dt + '&flag=1';
        }  
    });
    ...
    $(document).on('click', '.js-mytask', function () {
        window.location.href = 'task-management.html' + (type ? '?type=1' : '');
    });
    function init () {
