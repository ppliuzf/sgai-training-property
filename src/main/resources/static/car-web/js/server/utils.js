    ...
    $.extend({
        ...
        getData: function(config, callback, error) {
            var setting = {
                ...
                headers: {
                    ...
                    //'accessToken': 'PC_eyJhbGciOiJIUzI1NiJ9.eyJqdGkiOiJqd3QiLCJpYXQiOjE1Mjk2Mjg2OTcsInN1YiI6IntcInVzZXJUeXBlXCI6XCJJXCIsXCJjb21Db2RlXCI6XCJzazAwMVwiLFwidXNlck5hbWVcIjpcInNrdXNlcjEwXCIsXCJ1c2VySWRcIjpcInNrdXNlcjEwXCJ9IiwiZXhwIjoxNTI5NjcxODk3fQ.IZpSlpeccAlTdOOWKh1y80RVc-tM1VE-RYt5Py4g6go',
        ...
        kmCount: function (config) {
            ...
            $(opt.el).each(function () {
                var $this = $(this);
                $this.parent().find(opt.number).text($this.val().length +'/'+ opt.max);
                $this.off('keydown keyup keypress input').on('keydown keyup keypress input', function () {
                    var v = $this.val();
                    if (v.length > opt.max) {
                        $this.val(v.substr(0, opt.max));
                    }
                    var len = $this.val().length;
                    $this.parent().find(opt.number).text(len +'/'+ opt.max);
                    opt.callback(len, $this);
                });
            });
        },
        /**
        ...
        weekSelector: function(config) {
            var settings = {
                    el: '.week-selector', //div名字
                    fillZero: false, //自动补零 有小bug
                    curr: '', //起始时间
                    onselect: function() {}
                },
                opt = $.extend(settings, config);
            ...
            function flap(isPrev) {
                ...
                setTimeout(function() {
                    ...
                    el[selected[index] === currentTime ? 'addClass' : 'removeClass']('today');
                }, 250);
                setTimeout(function() {
                    el.removeClass('flap');
                }, 500);
            ...
            $('li', opt.el).off('click').on('click', function() {
                ...
                var todayTime = TodayDate.getTime(),
                    ...
                    willSelectTime = selected[$(this).index()]; // 点击的时间点
                if (willSelectTime > todayTime && willSelectTime < (todayTime + threeDay)) {
                    // 点击的时间点在三天内
                    selectedDate = willSelectTime; // 改变选择的时间点
        ...
        initTimeList: function() {
            ...
            function zeroFill(num) { // 在小于9的后面加0
                return num > 9 ? (num + '') : ('0' + num);
            }
            var j = 0;
            var nD = new Date()
            var hou = nD.getHours() + 1
            for (var i = 0; i <= 23; i++) { //i变量可以设置时间的节点
                timeList.push({
                    startTime: zeroFill(i) + ':00',
                    endTime: (zeroFill(i) + 1) + ':00' //整点时间
                        //endTime: zeroFill(i) + ':30'
                });
                // timeList.push({
                //     startTime: zeroFill(i) + ':30',
                //     endTime: zeroFill(i + 1) + ':00'
                // });
                j++;
            }
            return timeList;
